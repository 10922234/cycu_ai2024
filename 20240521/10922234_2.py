...
# 建立資料夾
os.makedirs('C:\\Users\\waes3\\Downloads\\M05A0429', exist_ok=True)
...
for i in range(24):
    for j in range(0, 60, 5):
        ...
        # 儲存 CSV 檔案
        df.to_csv(f"C:\\Users\\waes3\\Downloads\\M05A0429\\{i:02d}{j:02d}.csv", index=False)
        # 列印進度
        print(f"{i:02d}:{j:02d}")


# 合併所有 CSV 檔案
dfs = [pd.read_csv(f"C:\\Users\\waes3\\Downloads\\M05A0429\\{filename}") for filename in os.listdir("C:\\Users\\waes3\\Downloads\\M05A0429") if filename.endswith('.csv')]
df = pd.concat(dfs, ignore_index=True) if dfs else print("在指定的目錄下沒有找到任何 CSV 檔案。")

# 讀取和處理其他 CSV 檔案
df2 = pd.read_csv(r"C:\Users\waes3\Downloads\國道計費門架座標及里程牌價表104.09.04版_373038.csv", encoding="cp950", usecols=["編號", "緯度(北緯)", "經度(東經)"])
...
# 存檔並開啟
map.save(r"C:\Users\waes3\Downloads\2-2.html")
os.system(r"C:\Users\waes3\Downloads\2-2.html")